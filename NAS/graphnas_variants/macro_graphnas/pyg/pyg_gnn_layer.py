...
class GeoLayer(MessagePassing):
    ...
    def __init__(self,
        ...
        self.weight = Parameter(
            torch.Tensor(in_channels, heads * self.out_channels)) #might need to change to embed_dim
        self.att = Parameter(torch.Tensor(1, heads, 2 * 256))
        ...
        if self.att_type in ["generalized_linear"]:
            self.general_att_layer = torch.nn.Linear(self.out_channels, 1, bias=False)
        ...
        if pool_dim != 0:
            ...
            self.pool_layer.append(torch.nn.Linear(256, self.pool_dim))
            self.pool_layer.append(torch.nn.Linear(self.pool_dim, 256))
        else:
            pass
    ...
    def forward(self, x, edge_index, edge_embed):
        ...
        x = x.view(-1, self.heads, x.shape[-1])
        edge_embed = edge_embed.view(-1, self.heads, edge_embed.shape[-1])
        return self.propagate(edge_index, x=x, num_ndodes=x.size(0), edge_embed=edge_embed)

    def message(self, x_i, x_j, edge_index, num_nodes, edge_embed):
        ...
        if self.att_type == "const":
            ...
            alpha = self.apply_attention(edge_index, num_nodes, x_i, x_j, edge_embed)
            alpha = softmax(src=alpha, index=edge_index[0], num_nodes=num_nodes)
        ...
        return self.msg_encoder(neighbor + edge_embed) 

    def apply_attention(self, edge_index, num_nodes, x_i, x_j, edge_embed):
